...
const THEME_STORAGE_KEY = "coding-magic-theme";
...
function initGuessNumber(section) {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (guess === secret) {
      ...
      result.textContent = guess < secret ? "Спробуйте більше число." : "Спробуйте менше число.";
    }
  });
}
...
function initCalculator(section) {
  ...
  $("[data-calc-go]", section).addEventListener("click", () => {
    ...
    const answers = { add: a + b, subtract: a - b, multiply: a * b, divide: b === 0 ? "Помилка" : a / b };
...
function initDino(section) {
  ...
  function getScaleRatio() {
    ...
    if (screenWidth / screenHeight < GAME_WIDTH / GAME_HEIGHT) return screenWidth / GAME_WIDTH;
...
function renderThreeNumbers() {
  return `<div class="three-numbers">
    <input class="three-numbers__input" data-num="1" type="number" placeholder="Введіть число" />
    <input class="three-numbers__input" data-num="2" type="number" placeholder="Введіть число" />
    <input class="three-numbers__input" data-num="3" type="number" placeholder="Введіть число" />
  </div>
...
function renderScientists() {
  return `<div class="scientists">
    ...
    <div class="scientists__row">
      <button class="scientists__btn" data-sci="born19" type="button">Які вчені народилися в 19 ст.</button>
      <button class="scientists__btn" data-sci="einstein" type="button">Знайти рік народження Albert Einstein</button>
    </div>
    <div class="scientists__row">
      <button class="scientists__btn" data-sci="alphabet" type="button">Відсортувати вчених за алфавітом</button>
      <button class="scientists__btn" data-sci="c" type="button">Знайти вчених, прізвища яких починаються на літеру "C"</button>
    </div>
    <div class="scientists__row">
      <button class="scientists__btn" data-sci="age" type="button">Відсортувати вчених за кількістю прожитих років</button>
      <button class="scientists__btn" data-sci="removeA" type="button">Видалити всіх вчених, ім'я яких починається на "A"</button>
    </div>
    <div class="scientists__row">
      <button class="scientists__btn" data-sci="latest" type="button">Знайти вченого, який народився найпізніше</button>
      <button class="scientists__btn" data-sci="longest" type="button">Знайти вченого, який прожив найдовше і найменше</button>
    </div>
    <button class="scientists__btn scientists__btn--last" data-sci="same" type="button">
...
function initScientists(section) {
  ...
  function show(list) {
    grid.innerHTML = list.map((s) => `<div class="scientists__card">${s[0]} ${s[1]}<br>${s[2]}–${s[3]}</div>`).join("");
  }
  show(scientists);
  const actions = {
    born19: () => scientists.filter((s) => s[2] >= 1800 && s[2] < 1900),
    einstein: () => scientists.filter((s) => s[1] === "Einstein"),
    alphabet: () => [...scientists].sort((a, b) => a[1].localeCompare(b[1])),
    c: () => scientists.filter((s) => s[1].startsWith("C")),
    age: () => [...scientists].sort((a, b) => (a[3] - a[2]) - (b[3] - b[2])),
    removeA: () => {
      scientists = scientists.filter((s) => !s[0].startsWith("A"));
      return scientists;
    },
    latest: () => [scientists.reduce((a, b) => (a[2] > b[2] ? a : b))],
    longest: () => {
      const sorted = [...scientists].sort((a, b) => (a[3] - a[2]) - (b[3] - b[2]));
      return [sorted[0], sorted[sorted.length - 1]];
    },
    same: () => scientists.filter((s) => s[0][0].toLowerCase() === s[1][0].toLowerCase()),
  };
...
/* ============ GAME_CONFIG ============ */
const GAME_CONFIG = {
  leapYear:          { title: "Перевір в який рік ти народився",       enabled: true, render: renderLeapYear,       init: initLeapYear },
  guessNumber:       { title: "Вгадай число, яке загадав комп'ютер",   enabled: true, render: renderGuessNumber,    init: initGuessNumber },
  rockPaperScissors: { title: "Камінь — ножиці — папір",              enabled: true, render: renderRPS,            init: initRPS },
  calculator:        { title: "Калькулятор",                          enabled: true, render: renderCalculator,     init: initCalculator },
  timeCalculator:    { title: "Калькулятор часу",                     enabled: true, render: renderTimeCalculator, init: initTimeCalculator },
  dino:              { title: "Google динозавр",                      enabled: true, render: renderDino,           init: initDino },
  football:          { title: "Футбол",                               enabled: true, render: renderFootball,       init: initFootball },
  threeNumbers:      { title: "Введіть 3 числа",                      enabled: true, render: renderThreeNumbers,   init: initThreeNumbers },
};
  ...
  <main class="site-main">
    <div class="site-main__content">
      ...
      <section class="game-section team" id="team">
        <h2 class="game-section__title">Наша команда</h2>
        <div class="team-slider">
          <button class="team-slider__arrow" id="team-prev" type="button">‹</button>
          <div class="team-slider__card">
            <div class="team-slider__photo" id="team-photo"><span class="team-slider__photo-placeholder">Фото</span></div>
            <h3 class="team-slider__name" id="team-name"></h3>
            <p class="team-slider__description" id="team-description"></p>
            <div class="team-slider__dots" id="team-dots"></div>
          </div>
          <button class="team-slider__arrow" id="team-next" type="button">›</button>
        </div>
      </section>

      <section class="game-section game-section--dynamic" id="game-scientists">
        <h2 class="game-section__title">Обери вченого/их</h2>
        <div class="game-section__body"></div>
      </section>
    </div>
  </main>
...
themeToggle.checked = localStorage.getItem(THEME_STORAGE_KEY) === "dark";
...
/* ============ РЕНДЕР ИГР ============ */
function renderGameSection(id, game) {
  const section = document.createElement("section");
  section.className = "game-section game-section--dynamic";
  section.id = `game-${id}`;
  section.dataset.gameId = id;
  section.innerHTML = `<h2 class="game-section__title">${game.title}</h2><div class="game-section__body"></div>`;
...
function renderGameMenu() {
  $("#games-menu").innerHTML = `
    <a class="site-nav__menu-link" href="#game-leapYear">Числовий</a>
    <a class="site-nav__menu-link" href="#game-rockPaperScissors">Ігровий</a>
    <a class="site-nav__menu-link" href="#team">Ознайомчий</a>`;
}

function renderGames() {
  const list = $("#games-list");
  list.innerHTML = "";
  Object.entries(GAME_CONFIG).forEach(([id, game]) => {
    if (game.enabled) list.append(renderGameSection(id, game));
  });
}

function setGameEnabled(gameId, enabled) {
  if (!GAME_CONFIG[gameId]) return console.warn(`Гру "${gameId}" не знайдено.`);
  const oldSection = document.getElementById(`game-${gameId}`);
  if (oldSection && oldSection._dinoCleanup) oldSection._dinoCleanup();
  GAME_CONFIG[gameId].enabled = Boolean(enabled);
  renderGames();
}

window.CodingMagic = {
  games: {
    enable: (id) => setGameEnabled(id, true),
    disable: (id) => setGameEnabled(id, false),
    toggle: (id) => setGameEnabled(id, !GAME_CONFIG[id]?.enabled),
    state: () => Object.fromEntries(Object.entries(GAME_CONFIG).map(([id, g]) => [id, g.enabled])),
    render: renderGames,
  },
};
...
function renderTeamSlider() {
  $("#team-dots").innerHTML = teamMembers.map((_, i) =>
    `<button class="team-slider__dot ${i === currentMember ? "team-slider__dot--active" : ""}" data-team-dot="${i}" type="button"></button>`
  ).join("");
  $$("#team-dots [data-team-dot]").forEach((dot) => {
    dot.addEventListener("click", () => { currentMember = Number(dot.dataset.teamDot); showMember(); });
  });
  showMember();
}
function showMember() {
  const m = teamMembers[currentMember];
  $("#team-photo").innerHTML = m.image
    ? `<img class="team-slider__photo-image" src="${escapeHTML(m.image)}" alt="${escapeHTML(m.name)}" />`
    : `<span class="team-slider__photo-placeholder">Фото</span>`;
  $("#team-name").textContent = m.name;
  $("#team-description").textContent = m.description;
  $$("#team-dots [data-team-dot]").forEach((dot, i) =>
    dot.classList.toggle("team-slider__dot--active", i === currentMember)
  );
}
...
const sciSection = $("#game-scientists");
if (sciSection) {
  sciSection.querySelector(".game-section__body").innerHTML = renderScientists();
  initScientists(sciSection);
}
